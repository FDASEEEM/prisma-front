...
const FloatingSessionIndicator = () => {
  ...
  if (phase !== 'running' && phase !== 'awaiting_hitl') return null;
  ...
  return (
    <button
      ...
      className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full ${config.bg} shadow-lg ${config.shadow} flex items-center justify-center ring-2 ${config.ring} transition-all duration-300 group`}
    >
      <span
        className={`material-symbols-outlined text-white text-2xl ${!isHitl ? 'animate-spin' : ''}`}
        style={!isHitl ? { animationDuration: '2s' } : {}}
      >
        {config.icon}
      </span>
